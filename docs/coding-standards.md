...
- `src/providers/*`: optional client providers such as auth or theme wrappers; this directory is currently scaffold-only, and providers should be scoped to the smallest subtree that needs them.
...
- Put TanStack Query hooks in `src/hooks/queries/*` when client-side cache or revalidation is actually needed; the current repo does not have active query hooks yet.
